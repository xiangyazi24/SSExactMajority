import SSExactMajority.UpperBound.Time.DrainNoWakeProtocol
...
theorem wakeDelaytimerStepOK_PEM_of_all_resetting
    ...
    WakeDelaytimerStepOK Dmax γ t C
  ...
  · subst a
    ...
    · by_cases hsmall : (C (γ t).1).1.delaytimer ≤ 1
      ...
      · have hgt : 1 < (C (γ t).1).1.delaytimer := by omega
        ...
        · left
          simpa [Config.step, huv, PEMProtocol] using hfresh
        · right
          constructor
          · exact hall (γ t).1
          · have hsel : selectedAt γ (γ t).1 t := Or.inl rfl
  ...
  · by_cases hav : a = (γ t).2
    · subst a
      ...
      · have hgt : 1 < (C (γ t).2).1.delaytimer := by omega
        ...
        rcases hor with hfresh | hdrop
        · left
          simpa [Config.step, huv, hau, PEMProtocol] using hfresh
        · right
          constructor
          · exact hall (γ t).2
          · have hsel : selectedAt γ (γ t).2 t := Or.inr rfl
...
theorem wake_load_certificate_PEM_on_no_wake_prefix
    ...
    (γ : DetScheduler n)
    (hFresh :
      ∀ a, (C0 a).1.role = .Resetting ∧ (C0 a).1.delaytimer = Dmax)
    (hNoAwakePrefix :
      ∀ s, s ≤ K →
        ¬ SomeAgentAwake
  ...
  | zero =>
      ...
      intro a _ha
      exact le_of_eq (hFresh a).2.symm
  | succ t ih =>
      intro htK
